                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                              {featuredGames.map((game) => {
                                ...
                                const post: ContentPost = {
                                  ...
                                  title: game.gameName,
                                  slug: game.id,
                                  thumbnail: game.thumbnail || game.imageLink || '',
                                  banner: game.imageLink || game.thumbnail || '',
                                  description: game.description || (game.developerName ? `Developer: ${game.developerName}` : ''),
                                  shortDescription: game.shortDescription || (game.developerName ? `Developer: ${game.developerName}` : ''),
                                  author: game.developerName || '',
                                  developerName: game.developerName || '',
                                  developerEmail: game.developerEmail || '',
                                  developerWebsite: game.developerWebsite || '',
                                  websiteLink: game.gameLink || '',
                                  version: game.version || '',
                                  updatedDate: game.updatedDate || '',
                                  promptLink: targetPromptLink,
                                  publishDate: game.publishDate || '',
                                  category: game.category || 'FEATURED GAME',
                                  tags: Array.isArray(game.tags) ? game.tags : (typeof game.tags === 'string' && game.tags ? game.tags.split(',').map(t => t.trim()).filter(Boolean) : []),
